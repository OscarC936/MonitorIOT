...
//momento de conectarnos a db, localhost pues estoy en el mismo server
...
//para que se genere el usuario
  if ($password==$password_r){
    ...
    $users = $result->fetch_all(MYSQLI_ASSOC); // esta linea transforma el resultado en array

    //cuento cuantos elementos tiene $tabla,
    $count = count($users);

    //solo si no hay un usuario con mismo mail, procedemos a insertar fila con nuevo usuario
    if ($count == 0){ // es decir no encontro a nadie con ese mail
      //$password = sha1($password); //encriptar clave con sha1

      //ahora invoco conexion a DB y hago query
      $conn->query("INSERT INTO `users` (`users_email`, `users_password`) VALUES ('".$email."', '".$password."');");
      $msg.="Usuario creado correctamente, ingrese haciendo  <a href='login.php'>clic aquí</a> <br>"; // paso link a login
    }else{
...
<!--// empieza formulario-->
      <form method="post" target="register.php" name="form">
        <div class="md-form-group">
          <input name="email" type="email" class="md-input" value="<?php echo $email; ?>" required> <!--// para repoblar el formulario con el mail-->
          <label>Email</label>
        </div>
        <div class="md-form-group">
